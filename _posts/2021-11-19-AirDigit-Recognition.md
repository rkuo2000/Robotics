...
2. 於Kaggle上執行AirDigit-CNN進行模型訓練（點選上方[Run-All]來執行所有程式）
...
最後顯示辨識出之數字為**0**及回傳"POST /predict HTTP/1.1 200"訊息至手機<br />
